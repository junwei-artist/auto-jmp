...
get_port() {
    ...
    # Save the final port to config file (only if it changed or file doesn't exist)
    # Preserve existing settings and only update BACKEND_PORT
    if [ ! -f "$CONFIG_FILE" ]; then
        # Create new config file with all defaults
        echo "BACKEND_PORT=$port" > "$CONFIG_FILE"
        echo "BACKEND_HOST=0.0.0.0" >> "$CONFIG_FILE"
        echo "BACKEND_RELOAD=true" >> "$CONFIG_FILE"
    else
        # Update only BACKEND_PORT, preserve other settings
        if grep -q "^BACKEND_PORT=" "$CONFIG_FILE"; then
            # Update existing BACKEND_PORT only if it changed
            local current_port=$(grep "^BACKEND_PORT=" "$CONFIG_FILE" | cut -d'=' -f2)
            if [ "$current_port" != "$port" ]; then
                sed -i '' "s/^BACKEND_PORT=.*/BACKEND_PORT=$port/" "$CONFIG_FILE"
            fi
        else
            # Add BACKEND_PORT if it doesn't exist
            echo "BACKEND_PORT=$port" >> "$CONFIG_FILE"
        fi
        
        # Ensure other settings exist with defaults if missing
        if ! grep -q "^BACKEND_HOST=" "$CONFIG_FILE"; then
            echo "BACKEND_HOST=0.0.0.0" >> "$CONFIG_FILE"
        fi
        if ! grep -q "^BACKEND_RELOAD=" "$CONFIG_FILE"; then
            echo "BACKEND_RELOAD=true" >> "$CONFIG_FILE"
        fi
    fi
