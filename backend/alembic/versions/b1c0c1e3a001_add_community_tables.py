...
def upgrade() -> None:
    # Check if enum type already exists before creating it
    from sqlalchemy import text
    conn = op.get_bind()
    result = conn.execute(text("""
        SELECT EXISTS (
            SELECT 1 FROM pg_type WHERE typname = 'communityposttype'
        )
    """))
    enum_exists = result.scalar()
    
    if not enum_exists:
        op.execute("CREATE TYPE communityposttype AS ENUM ('question', 'tutorial', 'manual', 'sharing', 'tip', 'other')")
    ...
    op.create_table(
        ...
        sa.Column('type', sa.Enum('question', 'tutorial', 'manual', 'sharing', 'tip', 'other', name='communityposttype', create_type=False), nullable=False, server_default='sharing'),
