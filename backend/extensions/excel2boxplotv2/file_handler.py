"""
File Handler for Excel2BoxplotV2
...
class FileHandlerV2:
    """Handles Excel file loading and column exploration for V2"""
        ...
        try:
            ...
            try:
                ...
                if "expected <class 'int'>" in msg or 'xWindow' in msg or 'yWindow' in msg:
                    ...
                    raise
            logger.info(f"[V2] Found sheets: {self.sheets}")
            ...
            try:
                ...
                pass
            logger.info(f"[V2] Meta sheet loaded: {self.df_meta.shape}")

            # Load data
            self.df_data_raw = pd.read_excel(read_path, sheet_name="data")
            logger.info(f"[V2] Data sheet loaded: {self.df_data_raw.shape}")
            ...
            logger.error(f"[V2] Error loading Excel file: {str(e)}")
    ...
    def _fix_excel_file(self, file_path: str) -> Optional[str]:
        try:
            ...
            logger.info(f"[V2] Fixed Excel file written: {fixed_path}")
            return fixed_path
        except Exception as e:
            logger.error(f"[V2] Failed to fix Excel file: {e}")
            return None
    ...
    def _analyze_columns(self):
        ...
        # FAI columns detection stays the same
        self.fai_columns = [
        ...
        logger.info(f"[V2] FAI columns: {self.fai_columns}")
        logger.info(f"[V2] Categorical columns: {self.categorical_columns}")
