"""
Data Validator Module for Excel2BoxplotV2 Plugin
...
Note: V2 uses FileHandlerV2 to map meta columns to the internal standard:
  Y Variable->test_name, DETAIL->description, Target->target, USL->usl, LSL->lsl, Label->main_level
Therefore validation logic is identical to V1 after mapping.
"""
...
class DataValidator:
    """Validates Excel data structure and content (V2)"""
    ...
    def __init__(self):
        ...
        # V2 external/original column names before mapping
        self.v2_original_required = [
    ...
    def validate_structure(self, df_meta: pd.DataFrame, df_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        # Also record whether the original V2 column names were present prior to mapping
        original_cols = df_meta.attrs.get("original_columns") or []
        missing_original = [col for col in self.v2_original_required if col not in original_cols]
        if missing_original:
            warnings.append({
                "type": "missing_original_meta_columns",
                "message": f"Missing expected original meta columns (V2): {missing_original}",
                "details": {
                    "missing_original": missing_original,
                    "expected_original": self.v2_original_required,
    ...
    def validate_metadata_consistency(self, df_meta: pd.DataFrame, df_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        # Note: V2 uses 'Label' for group levels originally
        # We surface a hint if original 'Label' was not present.
        original_cols = df_meta.attrs.get("original_columns") or []
        if "Label" not in original_cols:
            warnings.append({
                "type": "original_label_missing",
                "message": "Original 'Label' column not found in meta (V2)",
    ...
    def validate_data_quality(self, df_meta: pd.DataFrame, df_data: pd.DataFrame, cat_var: str) -> Dict[str, Any]:
        ...
        # V2 categorical variable preference: 'Stage'
        preferred_cat = "Stage"
        ...
        if preferred_cat in df_data.columns and cat_var != preferred_cat:
            warnings.append({
                ...
                "message": f"Preferred categorical variable is '{preferred_cat}' for V2; using '{cat_var}'",
