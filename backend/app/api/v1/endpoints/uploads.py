...
async def file_serve_query(
    ...
    if not full_path.exists() and storage_key.startswith("tasks/"):
        ...
        relative_path = Path(storage_key[len("tasks/"):])  # Remove "tasks/" prefix
...
async def download_run_zip(
    ...
    async with AsyncSessionLocal() as db:
        ...
        if task_dir_rel:
            ...
            # Strip "tasks/" prefix since TASKS_DIRECTORY already points to the tasks folder
            tasks_dir = Path(settings.TASKS_DIRECTORY)
            if task_dir_rel.startswith("tasks/"):
                relative_path = task_dir_rel[len("tasks/"):]  # Remove "tasks/" prefix
                candidate = (tasks_dir / relative_path).resolve()
            else:
                candidate = (tasks_dir / task_dir_rel).resolve()
            if candidate.exists():
                ...
                if candidate.exists():
                    ...
                    # THIRD: Try under uploads as fallback
                    from app.core.storage import local_storage
