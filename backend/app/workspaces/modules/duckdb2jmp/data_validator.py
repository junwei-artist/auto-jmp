"""
Data Validator Module for DuckDB2JMP Module

Validates structure and ensures the DuckDB table matches expectations.
...
class DataValidator:
    """Validates DuckDB table data structure and content"""
    ...
    def validate_structure(self, df_meta: pd.DataFrame, df_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        if missing_columns:
            warnings.append({
                ...
                "message": f"Missing required columns in meta table: {missing_columns}",
                "details": {
        ...
        if empty_rows > 0:
            warnings.append({
                ...
                "message": f"Found {empty_rows} empty rows in meta table",
        ...
        if not fai_columns:
            warnings.append({
                "type": "no_fai_columns",
                "message": "No 'FAI' columns found in data table",
    ...
    def validate_metadata_consistency(self, df_meta: pd.DataFrame, df_data: pd.DataFrame) -> Dict[str, Any]:
            ...
            if "test_name" in df_meta.columns:
                ...
                if missing_in_data or extra_in_data:
                    warnings.append({
                        ...
                        "message": "Test name mismatch between meta and data tables",
                        "details": {
        ...
        # Only perform this check if 'main_level' column exists
        if 'main_level' not in df_meta.columns:
            # Skip metadata consistency check if main_level is missing
            pass
        else:
    ...
    def validate_data_quality(self, df_meta: pd.DataFrame, df_data: pd.DataFrame, cat_var: str) -> Dict[str, Any]:
        ...
        # Categorical variable preference: 'Stage' (case-insensitive)
        preferred_cat = "Stage"
        preferred_cat_found = None
        for col in df_data.columns:
            if str(col).lower() == "stage":
                preferred_cat_found = col
                break
        ...
        if preferred_cat_found and cat_var != preferred_cat_found:
            warnings.append({
                "type": "preferred_categorical_var",
                "message": f"Preferred categorical variable is '{preferred_cat_found}' (Stage); using '{cat_var}'",
                "details": {"preferred": preferred_cat_found, "selected": cat_var}
            })
