#!/usr/bin/env python3
"""
Test script for Excel2BoxplotV2 ZIP file generation with converted_2025-09-30_15-10-58.xlsx
"""
import requests
import json
import os
...
TEST_EXCEL_FILE = "/Users/lytech/Documents/GitHub/auto-jmp/excel-jsl-csv/converted_2025-09-30_15-10-58.xlsx"
...
def create_test_project(token):
    ...
    project_data = {
        "name": "BoxplotV2 ZIP Test",
        "description": "Testing ZIP file generation with BoxplotV2",
        "allow_guest": False,
        "is_public": False
    }
...
def test_boxplotv2_with_excel(token, project_id):
    """Test BoxplotV2 extension with converted Excel file"""
    print(f"\n🧪 Testing BoxplotV2 with converted Excel file...")
    ...
    with open(TEST_EXCEL_FILE, 'rb') as f:
        files = {'file': ('converted_2025-09-30_15-10-58.xlsx', f, 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')}
        data = {
            'project_id': project_id,
            'project_name': 'BoxplotV2 Test',
            'project_description': 'Testing ZIP generation with BoxplotV2',
            'cat_var': 'Stage',  # Common categorical variable
            'color_by': None
        }
        
        response = requests.post(
            f"{BACKEND_URL}/api/v1/extensions/excel2boxplotv2/run-analysis",
            files=files,
            data=data,
            headers=headers
        )
    ...
    if response.status_code == 200:
        ...
        print(f"✅ BoxplotV2 analysis completed")
        ...
        print(f"❌ BoxplotV2 analysis failed: {response.status_code} - {response.text}")
        return False
    ...
    if response.status_code == 200:
        ...
        with tempfile.NamedTemporaryFile(suffix='.zip', delete=False) as tmp_file:
            ...
            with zipfile.ZipFile(tmp_file.name, 'r') as zipf:
                ...
                expected_files = ['converted_2025-09-30_15-10-58.xlsx', 'data.csv', 'script.jsl', 'README.txt']
...
def main():
    """Main test function"""
    print("🚀 Testing BoxplotV2 ZIP file generation with converted Excel file")
    print("=" * 70)
    ...
    # Test BoxplotV2 with converted Excel file
    boxplotv2_success = test_boxplotv2_with_excel(token, project_id)
    ...
    print("\n" + "=" * 70)
    print("📊 Test Summary:")
    print(f"   BoxplotV2 Analysis: {'✅ PASS' if boxplotv2_success else '❌ FAIL'}")
    ...
    if all([boxplotv2_success, attachments_found, zip_verified]):
        print("\n🎉 All tests passed! BoxplotV2 ZIP generation is working perfectly.")
        print("   The ZIP file contains:")
        print("   - Original Excel file (converted_2025-09-30_15-10-58.xlsx)")
